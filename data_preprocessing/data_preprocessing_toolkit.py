...
class DataPreprocessingToolkit(object):
    ...
    def add_length_of_stay(self, df):
        ...
        t_diff = (df["date_to"] - df["date_from"]).apply(lambda x:x.days)
        df.loc[:,"length_of_stay"]=t_diff
#         display(df.head(10))
        return df
    ...
    def add_night_price(self, df):
        # Write your code here
        df.loc[:,"night_price"] = df.apply(lambda row: round(row["accomodation_price"]/(row["n_rooms"]*row["length_of_stay"]),2),axis=1)
        return df
    ...
    def map_night_price_to_room_segment_buckets(self, df):
        # Write your code here
        night_prices = df[df["accomodation_price"]>0].groupby(['room_group_id'])['night_price'].mean().reset_index()
        night_prices = night_prices.rename(columns={"night_price":"night_price_group"})
        df = pd.merge(df, night_prices, on=['room_group_id'],how='left')
        df.loc[:, 'room_segment'] = df['night_price_group'].apply(lambda x:self.map_value_to_bucket(x, self.room_segment_buckets))
        df = df.drop("night_price_group",1)
